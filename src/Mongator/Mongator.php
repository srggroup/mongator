...
class Mongator {
	...
	public static $doTranslate = false;
	 ...
	 *
	 * @param string $name The connection name.
	 * @return bool Returns if a connection exists.
	 */
	 ...
	 */
	public function create($documentClass, array $initializeArgs = []) {
		$document = new $documentClass($this);
		if (method_exists($document, 'initialize')) {
			call_user_func_array([$document, 'initialize'], $initializeArgs);
		}
