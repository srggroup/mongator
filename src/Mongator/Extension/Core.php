...
use Mongator\Query\CachedQuery;
...
class Core extends Extension {
	...
	private function parseAndCheckIdGeneratorProcess() {
		...
		if (!is_array($this->configClass['idGenerator'])) {
			...
			$this->configClass['idGenerator'] = ['name' => $this->configClass['idGenerator']];
		}

		if (!isset($this->configClass['idGenerator']['options'])) {
			$this->configClass['idGenerator']['options'] = [];
	...
	private function parseAndCheckReferencesProcess() {
		...
		foreach ($this->configClass['referencesOne'] as $name => $reference) {
			...
			$field = ['type' => 'raw', 'dbName' => $name, 'referenceField' => true];
	...
	private function checkDataNamesProcess() {
		foreach (
			array_merge(
				...
				!$this->configClass['isEmbedded'] ? array_keys($this->configClass['relationsOne']) : [],
				!$this->configClass['isEmbedded'] ? array_keys($this->configClass['relationsManyOne']) : [],
				!$this->configClass['isEmbedded'] ? array_keys($this->configClass['relationsManyMany']) : [],
				!$this->configClass['isEmbedded'] ? array_keys($this->configClass['relationsManyThrough']) : []
			) as $name
		) {
			if (in_array($name, ['Mongator', 'repository', 'collection', 'query_for_save', 'fields_modified', 'document_data'])) {
	...
	protected function initDefinitionsProcess() {
		...
		if (($pos = strrpos($classes['document'], '\\')) !== false) {
...
EOF
		...
		if ($this->configClass['isEmbedded']) {
			$definition->setParentClass(EmbeddedDocument::class);
		} elseif ($this->configClass['inheritance']) {
			$definition->setParentClass('\\' . $this->configClass['inheritance']['class']);
		} else {
			$definition->setParentClass(Document::class);
		}
		$definition->setDocComment(<<<EOF
/**
...
EOF
			...
			$definition->setAbstract(true);
			$definition->setParentClass(Repository::class);
			$definition->setDocComment(<<<EOF
/**
...
EOF
			...
			if ((int) $this->configClass['cache'] > 0) {
				$definition->setParentClass(CachedQuery::class);
			} else {
				$definition->setParentClass(Query::class);
			}
...
EOF
	...
	private function globalInheritableAndInheritanceProcess() {
		...
		foreach ($this->configClasses as $class => $configClass) {
			...
			foreach ($inheritedFields as $name => $field) {
				if (is_string($field)) {
					$inheritedFields[$name] = ['type' => $field];
				}
	...
	private function globalHasReferencesProcess() {
		...
		do {
			$continue = false;
			if (++$loop >= 10000) {
				throw new RuntimeException('preventing infinty loop in references, maybe typo or not defined class name.');
			}
	...
	private function globalMetadataProcess() {
		...
		$definition->setParentClass(MetadataFactory::class);
